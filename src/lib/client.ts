 ...
 * JSON bodies set Content-Type application/json.
 * FormData bodies omit Content-Type so the runtime sets the multipart boundary.
 */
async function request(method: Method, path: string, opts: RequestOptions = {}): Promise<unknown> {
  ...
  // Only set JSON content-type for JSON bodies — never for FormData
...
export const client = {
  ...
  /**
   * POST multipart FormData. Do NOT set Content-Type — Bun/fetch sets the
   * multipart boundary automatically.
   */
  postForm(path: string, formData: FormData, timeout?: number) {
    return request("POST", path, { formData, timeout });
  },
